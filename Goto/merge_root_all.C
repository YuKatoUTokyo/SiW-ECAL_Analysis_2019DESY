...
void merge_root_all(std::string str, bool filterEvents = false){
  ...
  for(Int_t islab=0; islab<MaxSlab; islab++){
    ...
    TString dif="";
    if(islab == 0) dif = "dif_1_1_1.raw";
    if(islab == 1) dif = "dif_1_1_2.raw";
    if(islab == 2) dif = "dif_1_1_3.raw";
    if(islab == 3) dif = "dif_1_1_4.raw";
    if(islab == 4) dif = "dif_1_1_5.raw";
    if(islab == 5) dif = "SLB_2";
    if(islab == 6) dif = "SLB_1";
    if(islab == 7) dif = "SLB_3";
    if(islab == 8) dif = "SLB_0";
    ...
    if(f[islab]->IsOpen()){
      ...
      if(islab<5) tree[islab] = (TTree*)f[islab]->Get("fev10");
      else tree[islab] = (TTree*)f[islab]->Get("slboard");
  ...
  while(continueScan && evt<MaxEvents){
    ...
    for(Int_t islab=0; islab<MaxSlab; islab++){
	  ...
	  for(int iraw=0; iraw<MaxMemory; iraw++) {
	    if(corrected_bcid_in[islab][ichip][iraw]<0 || bcid_in[islab][ichip][iraw]<0) continue;
	    if(islab<5) {
	      bcid_in[islab][ichip][iraw]=bcid_in[islab][ichip][iraw]-2492;
	      corrected_bcid_in[islab][ichip][iraw]=corrected_bcid_in[islab][ichip][iraw]-2492;
	    }
	  }
          Int_t imemory = 0;
          Int_t loopBCID = 0;
