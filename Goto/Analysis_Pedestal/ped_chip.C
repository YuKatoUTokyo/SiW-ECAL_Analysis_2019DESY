...
void ped_chip(std::string str){
  ...
  TString resultname = "./Chip_Result/" + filename_;
  ...
  // Fill to Tree Data
  Double_t pedestal_mean[5][15][64] = {-1};
  Double_t pedestal_width[5][15][64] = {-1};
  Double_t pedestal_error[5][15][64] = {-1};
  Double_t pedestal_chi2ndf[5][15][64] = {-1};

  // Create New TTree
  TFile *fout = new TFile(resultname + "_PedestalMap.root" , "RECREATE");
  TTree *Pedestal_Tree = new TTree("Pedestal_Tree", "Pedestal_Tree");
  Pedestal_Tree->Branch("most_hit_chips", most_hit_chips,
		        "most_hit_chips[5]/I");
  Pedestal_Tree->Branch("pedestal_mean", pedestal_mean,
		        "pedestal_mean[5][15][64]/D");
  Pedestal_Tree->Branch("pedestal_width", pedestal_width,
		        "pedestal_width[5][15][64]/D");
  Pedestal_Tree->Branch("pedestal_error", pedestal_error,
		        "pedestal_error[5][15][64]/D");
  Pedestal_Tree->Branch("pedestal_chi2ndf", pedestal_chi2ndf,
		        "pedestal_chi2ndf[5][15][64]/D");
    ...
    for(Int_t islab=0; islab<MaxSlab; islab++){
	  ...
	  for(Int_t ichn=0; ichn<MaxChannel; ichn++){
	    if(charge_lowGain[islab][ichip][imemory][ichn]>10 && gain_hit_low[islab][ichip][imemory][ichn]==0){
	      ped_sca.at(islab).at(ichn).at(imemory)->Fill(charge_lowGain[islab][ichip][imemory][ichn]);
	    }
	  }
	}
      }//imemory
  ...
  for(Int_t islab=0; islab<MaxSlab; islab++){
	...
	ped_sca.at(islab).at(ichn).at(imemory)->SetTitle(TString::Format("ped_slab%i_chip%i_chn%i_sca%i", islab, ichip, ichn, imemory));
	ped_sca.at(islab).at(ichn).at(imemory)->SetName(TString::Format("ped_slab%i_chip%i_chn%i_sca%i", islab, ichip, ichn, imemory));

	ped_mean.at(islab).at(ichn).push_back(0.);
	ped_width.at(islab).at(ichn).push_back(0.);
	ped_error.at(islab).at(ichn).push_back(0.);

	//====================================================================================================
	if(ped_sca.at(islab).at(ichn).at(imemory)->GetEntries()>10){ //max_entries/2 ){
	  TSpectrum *s = new TSpectrum();
	  int npeaks = s->Search(ped_sca.at(islab).at(ichn).at(imemory), 2, "", 0.2); 
	  ...
	  TH1 *pedestal_histogram = ped_sca.at(islab).at(ichn).at(imemory);
	  Double_t mean_peak = pedestal_histogram->GetBinCenter(pedestal_histogram->GetMaximumBin());
	    
	  TF1 *f0 = new TF1("f0", "gaus", mean_peak-2*ped_sca.at(islab).at(ichn).at(imemory)->GetRMS(), 
			      mean_peak+2*ped_sca.at(islab).at(ichn).at(imemory)->GetRMS());
	  ped_sca.at(islab).at(ichn).at(imemory)->Fit("f0", "RQNOC");

	  TF1 *f1 = new TF1("f1", "gaus", f0->GetParameter(1)-2.*f0->GetParameter(2), 
			      f0->GetParameter(1)+2.*f0->GetParameter(2));
	  ped_sca.at(islab).at(ichn).at(imemory)->Fit("f1","RQME");

  	  gStyle->SetOptFit(1111);
	  ped_sca.at(islab).at(ichn).at(imemory)->Write();
	  
	  ped_mean.at(islab).at(ichn).at(imemory)=f1->GetParameter(1);
	  ped_width.at(islab).at(ichn).at(imemory)=f1->GetParameter(2);
	  ped_error.at(islab).at(ichn).at(imemory)=f1->GetParError(1);

	  pedestal_mean[islab][imemory][ichn] = f1->GetParameter(1);
	  pedestal_width[islab][imemory][ichn] = f1->GetParameter(2);
	  pedestal_error[islab][imemory][ichn] = f1->GetParError(1);

	  if(imemory==0){
	    pedestal_map[islab]->Fill(map_pointX[ichip][ichn] , map_pointY[ichip][ichn] , pedestal_mean[islab][imemory][ichn]);
	    pedestal_width_map[islab]->Fill(map_pointX[ichip][ichn] , map_pointY[ichip][ichn] , pedestal_width[islab][imemory][ichn]);
	    pedestal_error_map[islab]->Fill(map_pointX[ichip][ichn] , map_pointY[ichip][ichn] , pedestal_error[islab][imemory][ichn]);
	  }  
  ...
  for(Int_t islab=0; islab<MaxSlab; islab++){
    ...
    pedestal_map[islab]->SetTitle("pedestal_map");
  ...
  for(Int_t islab=0; islab<MaxSlab; islab++){
    ...
    pedestal_width_map[islab]->SetTitle("pedestal_width_map");
    pedestal_map[islab]->GetXaxis()->SetTitle("x");
  ...
  for(Int_t islab=0; islab<MaxSlab; islab++){
    ...
    pedestal_error_map[islab]->SetTitle("pedestal_width_map");
    pedestal_map[islab]->GetXaxis()->SetTitle("x");
