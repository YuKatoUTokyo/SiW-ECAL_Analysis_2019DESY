...
void tdc_analysis(std::string str){
  ...
  for(Int_t i=0; i<9; i++){
    for(Int_t ichip=0; ichip<MaxChip; ichip++){
      for(Int_t ichn=0; ichn<MaxChannel; ichn++){
        tdc_vs_tdc[i][ichip][ichn] = new TGraph();
      }
    }
  }
    ...
    if(slab_number[0]==0&&slab_number[1]==1&&slab_number[2]==2&&slab_number[3]==3&&slab_number[4]==4&&nhit_channel<6&&bcid_hit%2==0){
      for(Int_t ichip=0; i<MaxChip; ichip++){
	    ...
	    if(channel_number[0]==ichn&&channel_number[1]==ichn&&channel_number[2]==ichn&&channel_number[3]==ichn&&channel_number[4]==ichn){
	      tdc_vs_tdc[0][ichip][ichn]->SetPoint(tdc_vs_tdc[0][ichip][ichn]->GetN(), charge_hiGain_tdc[0], charge_hiGain_tdc[1]);
	      tdc_vs_tdc[1][ichip][ichn]->SetPoint(tdc_vs_tdc[1][ichip][ichn]->GetN(), charge_hiGain_tdc[0], charge_hiGain_tdc[2]);
	      tdc_vs_tdc[2][ichip][ichn]->SetPoint(tdc_vs_tdc[2][ichip][ichn]->GetN(), charge_hiGain_tdc[0], charge_hiGain_tdc[3]);
	      tdc_vs_tdc[3][ichip][ichn]->SetPoint(tdc_vs_tdc[3][ichip][ichn]->GetN(), charge_hiGain_tdc[0], charge_hiGain_tdc[4]);
	      tdc_vs_tdc[4][ichip][ichn]->SetPoint(tdc_vs_tdc[4][ichip][ichn]->GetN(), charge_hiGain_tdc[1], charge_hiGain_tdc[2]);
	      tdc_vs_tdc[5][ichip][ichn]->SetPoint(tdc_vs_tdc[5][ichip][ichn]->GetN(), charge_hiGain_tdc[1], charge_hiGain_tdc[3]);
	      tdc_vs_tdc[6][ichip][ichn]->SetPoint(tdc_vs_tdc[6][ichip][ichn]->GetN(), charge_hiGain_tdc[1], charge_hiGain_tdc[4]);
	      tdc_vs_tdc[7][ichip][ichn]->SetPoint(tdc_vs_tdc[7][ichip][ichn]->GetN(), charge_hiGain_tdc[2], charge_hiGain_tdc[3]);
	      tdc_vs_tdc[8][ichip][ichn]->SetPoint(tdc_vs_tdc[8][ichip][ichn]->GetN(), charge_hiGain_tdc[2], charge_hiGain_tdc[4]);
	      tdc_vs_tdc[9][ichip][ichn]->SetPoint(tdc_vs_tdc[9][ichip][ichn]->GetN(), charge_hiGain_tdc[3], charge_hiGain_tdc[4]);
	    }
	  }
	}
      }
  ...
  for(Int_t i=0; i<9; i++){
    for(Int_t ichns=0; ichns<8; ichns++){
      canvas[i][ichns] = new TCanvas("canvas", "canvas", 1200, 1200);
      legend[i][ichns] = new TLegend(0.70, 0.01, 0.99, 0.39);
    }
  }
