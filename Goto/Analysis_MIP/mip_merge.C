...
void mip_merge(std::string str){
  ...
  canvas_mip_sca->Print(resultname + "_mip.pdf", "pdf");
 
  ofstream outtext("./Result/mip_mean.txt", ios::app);
  for(Int_t islab=0; islab<MaxSlab; islab++){
    outtext<<TString::Format("%i %i %i %f\n", islab, most_hit_chips[islab], most_hit_channels[islab], mip_mean[islab]);
  }
  outtext.close();
}
